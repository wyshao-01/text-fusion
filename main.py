...
def main():
    ...
    #
        ...
        test_imgs_ir_path= "./test_imgs/Roadscene/ir/"
        test_imgs_vis_path = "./test_imgs/Roadscene/vis/"
        print('TNO date set begin to test')
        result = "results"
        model_path = os.path.join(os.getcwd(), 'models_training', model_name)
        with torch.no_grad():
            ...
            for i in range(1000,1221):
                    index = i + 1
                    ir_path = test_imgs_ir_path +'roadscene_' +str(index) + ".png"
                    vis_path = test_imgs_vis_path+ 'roadscene_' + str(index) + ".png"
                    generate(model, ir_path, vis_path, model_path, index, mode='L')
            end = time.time()
            print("consumption time of generating:%s " % (end - begin))
